...
import { LoadingSpinner, EmptyState } from '@/components/catalog'
  ...
  useEffect(() => {
    async function fetchImages() {
      try {
        ...
        // Filter for image files and transform data
  ...
  return (
    <div className="min-h-screen bg-black relative pb-16">
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-100px] right-[-100px] w-[400px] h-[400px] rounded-full bg-[radial-gradient(circle,var(--pink)_0%,transparent_70%)] blur-[80px] opacity-10 animate-[float_20s_ease-in-out_infinite]" />
        <div className="absolute bottom-[-100px] left-[-100px] w-[350px] h-[350px] rounded-full bg-[radial-gradient(circle,var(--purple)_0%,transparent_70%)] blur-[80px] opacity-10 animate-[float_20s_ease-in-out_infinite_10s]" />
      </div>
      ...
      <header className="relative z-10 flex items-center justify-between px-8 py-8 md:px-16 border-b border-white/5 backdrop-blur-xl bg-black/80">
        <Link
          href="/"
          className="flex items-center gap-2 text-light-gray text-sm hover:text-white transition-colors duration-300"
        >
      ...
      <div className="relative z-1 max-w-[1600px] mx-auto px-4 md:px-8 py-12">
        {loading ? (
          ...
          <EmptyState message="No images found in gallery." />
        ) : (
